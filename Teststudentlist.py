...
class TestStudentLists(TestCase):
    ...
    def test_add_student_check_student_in_list(self):
        TestClass = ClassList(2)
        TestClass.add_student('Test Student')
        self.assertIn('Test Student', TestClass.class_list)
        TestClass.add_student('Another Test Student')
        self.assertIn('Test Student', TestClass.class_list)
        self.assertIn('Another Test Student', TestClass.class_list)
...
def test_add_remove_student_ensure_removed(self):
        test_class = ClassList(2)
        test_class.add_student('Mina Yimam')
        test_class.remove_student('Mina YImam')
...
def test_add_new_remove_non_exist_student(self):
    ...
    test_class.add_student('Test Student')
    test_class.add_student('Nahom Desta')
...
def test_removeUnknownStudent(self):
    test_class = ClassList(2)
    with self.assertRaises(StudentError):
        test_class.remove_student('Unknown Student')
...
## TODO write a test for index_of_student to assert it returns None if the
# student is not in the list if the list is empty. use assertIsNone.
def test_student_not_in_index_of_student_empty_list(self):
    test_class = ClassList(2)
    self.assertIsNone(test_class.index_of_student('Abir'))
...
def test_index_of_student_not_in_full_list(self):
    test_class = ClassList(3)
    test_class.add_student('Abir')
    test_class.add_student('Nahom')
    test_class.add_student('Selomie')
